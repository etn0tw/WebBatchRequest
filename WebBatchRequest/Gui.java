...
public class Gui extends JFrame {
    private JTextField uaText,dataText, timeoutText;
    private JMenuBar fileMenu,helpMenu,proxyMenu;
    private JMenuItem importTargetsMenuItem,exportMenuItem,aboutMenuItem,httpMenuItem,importpathMenuItem;
    ...
    private JTextArea targetsTextArea,headTextArea,cookiesTextArea,pathTextArea;
    private JScrollPane targetsScroll,tableScroll,pathScroll;
    ...
    public Gui()
    {
        ...
        importpathMenuItem = new JMenuItem("导入路径");
...
//            gridBagConstraints.fill=GridBagConstraints.BOTH;
        ...
        timeoutText = new JTextField(12);
        timeoutText.setPreferredSize(new Dimension (500,26));
        timeoutText.addFocusListener(new DefaultText(" 超时时间，默认为3秒",timeoutText));
        ...
        targetsTextArea = new JTextArea(8,24);
        ...
        pathTextArea = new JTextArea(8,25);
        pathTextArea.addFocusListener(new DefaultTextarea("默认为空", pathTextArea));
        pathScroll = new JScrollPane(pathTextArea);
...
//            westPanel.add(targetsPathText);
        ...
        // 创建表格模型,并且修复排列不正确的问题。
        dataModel = new DefaultTableModel(columnNames, 0){public Class<?> getColumnClass(int column) {
...
// ---------------------事件处理---------------------------
        ...
        runButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                String[] targets = targetsBody.split("\n");
                String headBody = headTextArea.getText();
                String targetsPathBody = pathTextArea.getText();
                String[] paths = targetsPathBody.split("\n");
                ...
                if (targetsPathBody.equals("默认为空")){
                    ...
                    for (String target : targets) {
                        for (String path: paths) {
                            if (target.startsWith("http://") || target.startsWith("https://")) {
                                str2.add(target + "/" + path);
                            } else {
                                str2.add("https://" + target + "/" + path);
                                str2.add("http://" + target + "/" + path);
                            }
                        }
                    }
                }
                ...
                if (postRadio.isSelected()){
                    ...
                    httpData(str2,cookiesBody,uaBody,headBody,"post",dataBody,enctypeBody.toString(),timeoutint);
                }
                if (headRadio.isSelected()){
                    httpData(str2,cookiesBody,uaBody,headBody,"head",null,null,timeoutint);
                }
                if (getRadio.isSelected()){
                    httpData(str2,cookiesBody,uaBody,headBody,"get",null,null,timeoutint);
                }
...
//
    ...
    public void httpData(ArrayList<String> str, final String cookiesBody, final String uaBody, final String headBody, final String methods, final String dataBody, final String enctypeBody,final int timeout){
        // 获取线程池数量
...
//                            if (target.startsWith("http://") || target.startsWith("https://")) {
                        if (proxyPower == false){
                            responseData = Http.Response(target, cookiesBody, uaBody, headBody, methods, dataBody, enctypeBody,follow,timeout);
                        }else {
                            System.out.println(host);
                            System.out.println(port);
                            responseData = Http.Response(target, cookiesBody, uaBody, headBody, methods, dataBody, enctypeBody,follow,host,port,timeout);
                        }
...
//                                list.addElement(responseData.get(1));
                        ...
                        e.printStackTrace();
                        list.addElement(target);
                        //由于排序时状态码 一般为数字型，格式不对会导致排序出错，故注释。
...
//                            list.addElement("<html><span style=\"color: red;\">"+"无法访问"+"</span></html>");
    ...
    class about implements ActionListener{
        @Override
        public void actionPerformed(ActionEvent e){
            JOptionPane.showMessageDialog(null, "WEB批量请求器 \nVersion 2.0\nOriginal Author：@xiaowei\nAuthor：etn0tw\nGithub: https://github.com/etn0tw/WebBatchRequest\nCopyright 2024", "关于",JOptionPane.WARNING_MESSAGE);//弹框提示
        }
    }
