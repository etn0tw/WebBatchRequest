    ...
    public static ArrayList Response (String str,String cookie, String ua,String xHeaders,String methods,String dataBody,String enctypeBody,boolean follow,int timeout){
        ...
        Map<String, String> headers = new HashMap<String, String>();
            ...
            {   headers.put("Content-type",enctypeBody);
                response = HttpRequest.post(str).headers(headers).trustAllHosts().trustAllCerts().send(dataBody).followRedirects(follow).connectTimeout(timeout*1000);

            }
            if (methods == "head"){
                response = HttpRequest.head(str).headers(headers).trustAllCerts().trustAllHosts().followRedirects(follow).connectTimeout(timeout*1000);
            }
            if(methods == "get"){
                response = HttpRequest.get(str).headers(headers).trustAllHosts().trustAllCerts().followRedirects(follow).connectTimeout(timeout*1000);
            }
        code = response.code();
        body = response.body();
...
//        System.out.println(!"SESSION=A202106084F2...".equals(cookie));
        ...
        if (methods == "post")
        {   headers.put("Content-type",enctypeBody);
            response = HttpRequest.post(str).useProxy(host, port).headers(headers).trustAllHosts().trustAllCerts().send(dataBody).followRedirects(follow).connectTimeout(timeout*1000);
        }
        if (methods == "head"){
            response = HttpRequest.head(str).useProxy(host, port).headers(headers).trustAllCerts().trustAllHosts().followRedirects(follow).connectTimeout(timeout*1000);
        }
        if(methods == "get"){
            response = HttpRequest.get(str).useProxy(host, port).headers(headers).trustAllHosts().trustAllCerts().followRedirects(follow).connectTimeout(timeout*1000);
        }
        code = response.code();
        body = response.body();
        banner = response.header("server");
